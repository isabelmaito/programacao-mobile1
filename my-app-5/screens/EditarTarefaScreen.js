...
export default function EditarTarefaScreen({ route, navigation }) {
  ...
  if (!tarefa) return <View style={styles.container}><Text>Tarefa inválida.</Text></View>;
  ...
  const atualizar = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/tarefas/${tarefa.id}`, { descricao, status });
      navigation.goBack();
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={[styles.salvar, { backgroundColor: '#ff9800' }]} onPress={atualizar}>
        <Text style={styles.salvarTxt}>Atualizar</Text>
      </TouchableOpacity>
    </View>
  );
}
